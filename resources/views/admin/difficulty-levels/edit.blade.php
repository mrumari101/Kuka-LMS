...
@section('title', 'Dashboard - Difficulty Level')
...
@section('content')
    ...
    <x-breadcrumb
        ...
        :items="[
                ...
                ['label' => 'Difficulty Level', 'url' => route('admin.difficulty-levels.index')],
                ['label' => 'Update', 'url' => route('admin.difficulty-levels.edit',['difficultyLevel'=>$difficultyLevel->id])]
            ]"
    />
        ...
        <div class="card-header">
            <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                    <h5 class="mb-0" data-anchor="data-anchor" id="basic-form">Edit Difficulty Level<a class="anchorjs-link " aria-label="Anchor" data-anchorjs-icon="#" href="#basic-form" style="margin-left: 0.1875em; padding-right: 0.1875em; padding-left: 0.1875em;"></a></h5>
                </div>
        ...
        <div class="card-body bg-body-tertiary">
            <div class="tab-content">
                <div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-43631251-35c6-4416-9d8b-497c94bd83a2" id="dom-43631251-35c6-4416-9d8b-497c94bd83a2">
                    <form action="{{ route('admin.difficulty-levels.update', ['difficultyLevel' => $difficultyLevel->id]) }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label" for="name">
                                Name <span class="text-danger">*</span>
                            </label>
                            <input class="form-control @error('name') is-invalid @enderror" name="name" id="name" type="text" value="{{ old('name',$difficultyLevel->name) }}" placeholder="Name">
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" name="status" id="status" aria-label="Default select example">
                                ...
                                <option value="1" {{ old('status',$difficultyLevel->status) == '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('status',$difficultyLevel->status) == '0' ? 'selected' : '' }}>Unactive</option>
                            </select>
                            @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
