...
class QuestionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // MCQ fields
            'mcq_description' => 'nullable|string',
            'mcq_file' => 'nullable|file|mimes:pdf,doc,docx|max:10240',

            'description' => 'nullable|array|min:2|max:5',
            'description.*' => 'nullable|string|required_with:description',
            ...
            'solution_file' => 'nullable|file|mimes:pdf,doc,docx|max:10240',
        ];
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($this->question_type === 'mcq') {
                ...
                // MCQ content required
                if (
                    empty($this->mcq_description) &&
                    !$this->hasFile('mcq_file')
                ) {
                    $validator->errors()->add(
                        'mcq_description',
                        'Either MCQ description or MCQ file is required.'
                    );
                }

                // Options count
                $options = $this->input('description', []);
                if (count($options) < 2) {
                    $validator->errors()->add(
                        'description',
                ...
                $correct = array_map('intval', $this->input('is_correct', []));
                if (array_sum($correct) !== 1) {
                    $validator->errors()->add(
                        'is_correct',
                        'Exactly one MCQ option must be marked correct.'
                    );
                }
            }
